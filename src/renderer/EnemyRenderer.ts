/**
 * EnemyRenderer - Pure rendering logic for Enemy entity.
 * Separated from Enemy class to enable testing of game logic without THREE.js.
 */
...
export class EnemyRenderer {
  ...
  constructor(config: EnemyRenderConfig) {
    this.mesh = new THREE.Group()
    this.buildModel(config)
  }

  private buildModel(config: EnemyRenderConfig): void {
    switch (config.type) {
  ...
  /**
   * Apply animation based on enemy type and animPhase
   */
  applyAnimation(type: EnemyType, animPhase: number): void {
    if (type === EnemyType.Slime) {
      const scale = 0.8 + Math.sin(animPhase) * 0.1
      const body = this.mesh.children[0] as THREE.Mesh
      if (body) body.scale.set(1, scale, 1)
    } else if (type === EnemyType.Bat) {
      const flap = Math.sin(animPhase * 2) * 0.3
      ...
      this.mesh.position.y = 0.3 + Math.sin(animPhase) * 0.2
    }
  }
   ...
   */
  applyDamageFlash(originalColor: number): void {
   ...
   */
  syncPosition(position: THREE.Vector3, rotation: number): void {
